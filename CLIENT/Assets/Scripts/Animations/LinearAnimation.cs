...
public class LinearAnimation : MonoBehaviour
{
    ...
    private void InitializeAnim(string type)
    {
        ...
        switch (type)
        {
            case "string":
                ...
                //Position elements
                textComponent.GetComponent<RectTransform>().localPosition = new Vector3(componentWidth/2, 0, 0);
                textClone.GetComponent<RectTransform>().localPosition = new Vector3(componentWidth/2, 0, 0);

              

                break;
            case "phra":
                PARENT.GetComponent<RectTransform>().sizeDelta = new Vector2(Screen.width, 300);
                componentWidth = PARENT.GetComponent<RectTransform>().rect.width / textOffset;
                RespawnPointsPosition(componentWidth);
                //Position elements
                textComponent.GetComponent<RectTransform>().localPosition = new Vector3(componentWidth / 2, 0, 0);
            ...
            case "image":
                ...
                PARENT.GetComponent<RectTransform>().sizeDelta = new Vector2(Screen.width, 800);
                componentWidth = PARENT.GetComponent<RectTransform>().rect.width / imageOffset;
                RespawnPointsPosition(componentWidth);

              
                //Position elements
                imageComponent.GetComponent<RectTransform>().localPosition = new Vector3(350, 0, 0);
    ...
    {
        A.GetComponent<RectTransform>().localPosition = new Vector3(-distance/2, 0, 0);
        B.GetComponent<RectTransform>().localPosition = new Vector3(distance/2, 0, 0);
    }
    public void SetString(StringMessage message)
    {
        ...
        textClone.font = fontAssets[GetFontAssetIndex(message.FontType)];
        textComponent.font = fontAssets[GetFontAssetIndex(message.FontType)];
    ...
    public IEnumerator SetImage(ImageMessage message)
    {
        ...
        cloneImage.texture = null;

        isTextRunning = false;
        isImageRunning = true;


        if (message.Type == "linear-static")
        {
