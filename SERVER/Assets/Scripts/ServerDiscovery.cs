...
*/
[Serializable]
public class ClientConnectionAttempt : UnityEvent<string> { };
public class ServerDiscovery : MonoBehaviour
{
    public ClientConnectionAttempt onClientConnectionRequested = null; 
        ...
        while (this.udpReceiver != null) // continue to receive data as long its existing
        {
            ...
            try
            {
                ...
                if (data.type == DiscoveryType.REQUEST_ALL || (data.type == DiscoveryType.REQUEST && supportedCommands.Contains(data.command)))
                {
                    ...
                    string txt = "Request from: " + src.Address.ToString();
                    Debug.Log(txt);
                    onClientConnectionRequested.Invoke(src.Address.ToString());
